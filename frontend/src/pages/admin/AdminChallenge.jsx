...
const GAME_TYPES = [
  { value: 'race', label: 'Course', emoji: '🏎️' },
...
function CreateChallengeForm({ gameType, onCreated, onCancel }) {
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 py-2.5 rounded-lg text-sm font-medium disabled:opacity-60 flex items-center justify-center gap-2 text-white transition-all hover:brightness-110"
...
function ActiveChallengePanel({ challenge, onEnded }) {
  ...
  const confirmEnd = async () => {
    ...
    setEnding(true);
    try {
      const res = await apiFetch(`${API_URL}/api/admin/challenge/${challenge.id}/end`, {
        method: 'POST',
      });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 items-start">
      <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
          <p className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>{challenge.title}</p>
          <span className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ backgroundColor: ACCENT, color: '#fff' }}>
            En cours
          </span>
        </div>
        <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
          Deadline : {new Date(challenge.deadline).toLocaleString('fr-FR')}
        </p>

        <div className="flex flex-col gap-4">
          {challenge.runners?.map((r) => (
            <div key={r.id}>
              <div className="flex items-center justify-between text-sm mb-1.5">
                <span style={{ color: 'var(--text-primary)' }}>{r.nom} {r.isWinner ? '🏆' : ''}</span>
                <span style={{ color: 'var(--text-muted)' }}>{r.total} / {challenge.target}</span>
              </div>
      ...
      <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        ...
        <div className="flex flex-col gap-2.5">
          {[
            { label: 'Appels', value: challenge.targets?.appel || 0 },
            { label: 'Rendez-vous', value: challenge.targets?.rdv || 0 },
            { label: 'Devis', value: challenge.targets?.devis || 0 },
            { label: 'Commandes', value: challenge.targets?.commande || 0 },
          ].map((t) => (
          ...
          <div className="flex items-center justify-between text-sm pt-2.5 mt-1" style={{ borderTop: '1px solid var(--border)' }}>
            <span style={{ color: 'var(--text-primary)' }}>Total</span>
            <span className="font-semibold" style={{ color: ACCENT }}>{challenge.target}</span>
          </div>
        </div>
      </div>
...
function GameTypeSlot({ gameInfo, challenge, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{gameInfo.label}</p>
      </div>

      {challenge ? (
        <ActiveChallengePanel challenge={challenge} onEnded={onRefresh} />
      ) : creating ? (
        <CreateChallengeForm
          gameType={gameInfo.value}
        ...
        <button
          ...
          className="rounded-2xl p-5 text-sm font-medium text-left transition-all hover:-translate-y-0.5"
          style={{ backgroundColor: 'var(--surface)', border: '1px dashed var(--border)', color: 'var(--text-secondary)' }}
        >
          + Lancer un défi {gameInfo.label.toLowerCase()}
        </button>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] p-4 sm:p-6 pb-12 relative overflow-hidden" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-8 relative z-10">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight" style={{ color: 'var(--text-primary)' }}>Gestion des défis</h1>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>Un défi actif possible par type de jeu — lance-en plusieurs en parallèle</p>
        </div>

        {GAME_TYPES.map((gameInfo) => (
          <GameTypeSlot
            key={gameInfo.value}
            gameInfo={gameInfo}
            challenge={challenges.find((c) => c.gameType === gameInfo.value)}
